...
def _maybe_trace_txtattn_last_row(attn_weights, config, layer_idx):
    ...
    if record["layout"] is None:
        generated_prefix_len = max(att_seq_len - generated_start, 0)
        record["layout"] = {
            "prompt_visible_len": int(img_start + 1 + prompt_after_image_len),
            "generated_len": int(generated_prefix_len),
            ...
            "trace_mode": "last_row",
            "trace_note": "Last-query-row trace; I_text and txt_img_ratio use the full text-side region image_end:, i.e. question prompt plus generated text.",
        }
...
def _apply_text_intervention(attn_weights, config, layer_idx):
    ...
    for head, head_score in selected:
        ...
        if mode in ("dynamic", "late_boost") and dynamic_redistribute in ("system", "system_only", "vision", "vision_only", "sysvis"):
    ...
    if mode not in ("dynamic", "late_boost") or dynamic_renorm:
...
class LlamaSdpaAttention(LlamaAttention):
    ...
    def forward(
        ...
        if intervention_active or trace_active:
            # Keep the SDPA path for the actual generation flow. This side-channel
            # materializes only the last query row, which is enough for txt-attn
            # tracing and for our last-token attention intervention.
