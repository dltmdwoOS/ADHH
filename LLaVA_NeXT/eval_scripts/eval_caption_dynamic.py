...
def default_head_setup(model_path):
    ...
    elif model_path in ("lmms-lab/llama3-llava-next-8b", "llama3-llava-next-8b"):
        return {
            "heads": [],
            # LLaVA-NeXT uses anyres image features, so these are placeholders.
            # llava_next.model.llava_arch updates them per sample before attention.
            "img_start_pos": 0,
            "img_length": 0,
        }
    else:
        if "llava-next" in model_path.lower() or "llama3-llava" in model_path.lower():
            return {"heads": [], "img_start_pos": 0, "img_length": 0}
...
def eval_model(args):
    ...
    for (input_ids, image_tensor, image_sizes), line in tqdm(
        ...
        ans_file.flush()
        del output_dict, output_ids, input_ids, image_tensor
