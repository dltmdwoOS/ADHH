...
from llava_next.constants import (
    DEFAULT_IMAGE_TOKEN,
...
def infer_step_masks(input_ids, generated_prefix_ids, att_seq_len, special_token_ids):
    ...
    txt_mask[image_end:att_seq_len] = True
            ...
            head_values.append(
                {
                    ...
                    "txt_img_ratio": i_text / (image_attn + eps),
                }
            )
...
def extract_txtattn_last_row_trace_for_sample(
    ...
    return {
        ...
        "num_records": int(num_records),
        "trace_mode": "last_row",
        "trace_note": "Lightweight trace captured from the last query row. I_text and txt_img_ratio use the full text-side region image_end:, i.e. question prompt plus generated text.",
    }
...
def build_token_mask_debug(tokenizer, input_ids, generated_ids_only, image_len, special_token_ids):
    ...
    return {
        "layout": {
            ...
            "image_len": int(image_len),
            "generated_start": int(generated_start),
            "sys_len": int(image_start),
            "txt_len": int(final_att_seq_len - generated_start + prompt_after_image_len),
...
def eval_model(args):
    ...
    for sample_idx, ((input_ids, image_tensor, image_sizes), line) in tqdm(
        ...
        output_attentions = args.enable_attention_analysis or (
            args.enable_txtattn_trace and args.txtattn_trace_mode == "full"
        )
        ...
        # Large attention outputs are kept in output_dict until explicitly
        # released. Free them before the next generate() call starts.
        del output_dict, output_ids, generated_ids_only, input_ids, image_tensor
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--txtattn-trace-mode",
        choices=["last_row", "full"],
        default="last_row",
        help="Use lightweight last-query-row tracing by default; use full to preserve the old output_attentions=True trace.",
    )
