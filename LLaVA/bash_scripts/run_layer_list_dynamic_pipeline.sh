...
IFS=';' read -r -a dynamic_presets <<< "${DYNAMIC_PRESETS:-1.0 9.0 1.0; 1.0 7.0 1.0; 1.0 6.0 1.0; 1.0 5.0 1.0;}"
...
if [[ ! -f "${source_summary}" ]]; then
  ...
  echo "Expected a layer-0-to-31 all-head txtattn_summary.json. Run decoding_base_with_original_qa.sh first." >&2
  exit 1
fi
