...
result_path=${RESULT_PATH:-${results_root}/amber/${model_name}/baselines/tarac/tok${max_new_tokens}/${sample_suffix}_alpha${alpha_slug}_beta${beta_slug}_l${start_layer}-${end_layer}}
mkdir -p "${result_path}"
...
if [[ "${dry_run}" == "true" ]]; then
  echo "[dry-run] AMBER TARAC would write -> ${result_path}"
...
echo "AMBER TARAC done -> ${result_path}"
