...
# The official PAI README uses alpha=0.2. Override PAI_ALPHA for ablations.
alpha=${PAI_ALPHA:-0.5}
gamma=${PAI_GAMMA:-1.1}
...
result_path=${RESULT_PATH:-${results_root}/${dataset}/${model_name}/baselines/pai/tok${max_new_tokens}/alpha${alpha}${cfg_suffix}_l${start_layer}-${end_layer}}
mkdir -p "${result_path}"
...
if [[ "${dry_run}" == "true" ]]; then
  echo "[dry-run] CHAIR PAI would write -> ${result_path}"
...
echo "CHAIR PAI done -> ${result_path}"
