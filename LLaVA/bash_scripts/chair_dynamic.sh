...
# text_exp uses c = exp(k * (I_text - tau)); ratio_power recovers the earlier dynamic.
...
run_job() {
  ...
  local result_path=./results_dynamic/${dataset}/${model_name}_dynamic_${dynamic_context_mode}_${head_source}_k${topk}_s${dynamic_strength}_q${dynamic_exp_sharpness}_tau${dynamic_tau}_p${dynamic_score_power}_n${num_samples}_${head_score_key}
  mkdir -p "${result_path}"
  ...
  echo "[GPU ${gpu}] Dynamic start: topk=${topk}, score=${head_score_key}/${head_score_normalize}, mode=${dynamic_context_mode}, strength=${dynamic_strength}, exp_sharpness=${dynamic_exp_sharpness}, tau=${dynamic_tau}, score_power=${dynamic_score_power}, use_head_scores=${use_head_scores}"
  ...
  echo "[GPU ${gpu}] Dynamic done: topk=${topk}, mode=${dynamic_context_mode}, strength=${dynamic_strength}, exp_sharpness=${dynamic_exp_sharpness}, tau=${dynamic_tau}, score_power=${dynamic_score_power}"
}
