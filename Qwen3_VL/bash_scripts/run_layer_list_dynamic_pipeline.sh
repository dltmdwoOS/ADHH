...
source_summary=${SOURCE_SUMMARY:-./results/${dataset}/${model_name}_base_original_qa_n${train_num_samples}_txtattn_l0_l27_allheads/txtattn_summary.json}
...
if [[ ! -f "${source_summary}" ]]; then
  ...
  echo "Run Qwen3_VL/bash_scripts/decoding_base_with_original_qa.sh first." >&2
  exit 1
fi

sample_dir=./results/${dataset}/shared_samples
sample_file=${sample_dir}/val_seed${seed}_n${num_samples}.json
mkdir -p "${sample_dir}"
...
run_dynamic_job() {
  ...
  echo "[GPU ${gpu}] done layers=${layer_spec} topk=${topk} s=${dynamic_strength} q=${dynamic_exp_sharpness} tau=${dynamic_tau}"
}
...
echo "Qwen layer-list dynamic pipeline finished."
